...
const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "Your text edits in the visual editor can no longer be silently lost: if the source file changed while you were typing (a formatter or hot reload touching it), Cripcode now re-finds your element and saves your text there — and tells you plainly in the rare case it can't",
      ...
      'When your system is briefly too busy to start a new process (the macOS "Resource temporarily unavailable" error), Cripcode now quietly retries instead of surfacing a scary one-off failure',
      ...
      '80+ auto-reported issues fixed in total across three bug-bash rounds — the largest cleanup in Cripcode history',
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      'Quitting Cripcode now really stops everything — dev servers and agent sessions no longer linger in the background after the app closes, and restarting a dev server reliably frees its port',
  ...
  },
  {
    ...
    items: [
      'Bugs now fix themselves faster: when something goes wrong, Cripcode automatically reports the error (anonymized — never your code or files) so it can be investigated and patched. Opt out anytime in Settings → Usage analytics & error reports',
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      'Agent-led onboarding — setting up a new machine? Pick your coding agent and it installs everything else for you (Homebrew, Node, Git, GitHub, your hosting CLI) while Cripcode independently verifies each step. The classic wizard stays one click away at all times',
  ...
  },
  {
    ...
    items: [
      ...
      "Per-project workspaces — assign any project to a workspace and its terminals, git, pull requests, and AI all use that workspace's logins automatically. Move a project between workspaces right from the dashboard and Cripcode relocates its files for you",
      ...
      'Choose your projects folder — point Cripcode at any folder you like (such as an existing ~/Dev directory) instead of ~/ShipStudio, globally or per workspace, and optionally move your existing projects across',
  ...
  },
  {
    ...
    items: [
      'Mobile app previews — open a React Native, Expo, or Flutter project and the preview pane becomes a real, interactive device. Cripcode boots an iOS Simulator or Android emulator for you, builds and launches your app onto it (with the build log streaming in), and mirrors the screen live so you can tap, swipe, and type right in the workspace',
      ...
      'Multilingual sites — Cmd+K → “Languages” to add languages to any Next.js or Astro site. Search every language (not a preset list), pick a default, and Cripcode edits your i18n config surgically — it never guesses, and unusual configs fall back to a “Fix with AI” hand-off instead of being overwritten',
  ...
  },
  {
    ...
    items: [
      ...
      "Restarting the dev server no longer crashes Cripcode — on some setups it was killing the app's own preview process along with the dev server",
  ...
  },
  {
    ...
    items: [
      ...
      'The toolbar Support button now opens the Cripcode community Slack directly',
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      'Monorepo support — Cripcode detects pnpm/npm workspaces when you import a repo (or first open an existing project) and asks which app to work on; the dev server, preview, and /public tools run inside that workspace while git and PRs stay at the repo root. "Use the whole repo" skips the picker',
  ...
  },
  {
    ...
    items: [
      ...
      'Open in Browser fix — Preview toolbar\'s "Open in Browser" now opens your real dev server URL (e.g. `localhost:3000`) instead of Cripcode\'s internal proxy port',
