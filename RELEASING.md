...
- https://github.com/fileboin/cripcode/releases
...
- [ ] Verify public URL works:
  ```bash
  curl -sL https://github.com/fileboin/cripcode/releases/latest/download/latest.json | jq
  ```
...
Windows builds run on a separate workflow (`.github/workflows/release-windows.yml`) triggered by tags ending in `-win`. The macOS workflow explicitly excludes these tags (`'!*-win'`), so the two build pipelines are independent, but both publish to the shared `fileboin/cripcode` public repo. As of v0.6.8 the Windows public release **auto-publishes**, exactly like macOS — there is no longer a manual publish step (see History below).
...
- [ ] `latest.json` still resolves at the public latest URL and still points at the most recent macOS bundle (the carry-forward keeps macOS auto-update alive when this release flips the "latest" alias):
  ```bash
  curl -sL https://github.com/fileboin/cripcode/releases/latest/download/latest.json | jq '.version'
  ```
...
1. Ensure the release in `fileboin/cripcode` is **published** (not draft)
